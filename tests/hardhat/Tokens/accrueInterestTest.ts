...
function getBlockNumberOrTimestamp(isTimeBased: boolean) {
  if (!isTimeBased) {
    return blockNumber;
  }
  ...
  describe(`${description}VToken`, () => {
    ...
    describe("accrueInterest", () => {
      it("reverts if the interest rate is absurdly high", async () => {
        await pretendBlockOrTimestamp(vToken, getBlockNumberOrTimestamp(isTimeBased), 1);
      ...
      it("fails if simple interest factor calculation fails", async () => {
        await pretendBlockOrTimestamp(vToken, getBlockNumberOrTimestamp(isTimeBased), parseUnits("5", 70));
      ...
      it("fails if new borrow index calculation fails", async () => {
        await pretendBlockOrTimestamp(vToken, getBlockNumberOrTimestamp(isTimeBased), parseUnits("5", 60));
        await expect(vToken.accrueInterest()).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_UNDER_OR_OVERFLOW);
      });
      ...
      it("fails if interest accumulated for reserves calculation fails", async () => {
        ...
        await pretendBlockOrTimestamp(vToken, getBlockNumberOrTimestamp(isTimeBased), parseUnits("5", 20));
        await expect(vToken.accrueInterest()).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_UNDER_OR_OVERFLOW);
      });
      ...
      it("fails if new total reserves calculation fails", async () => {
        ...
        await pretendBlockOrTimestamp(vToken);
        await expect(vToken.accrueInterest()).to.be.revertedWithPanic(PANIC_CODES.ARITHMETIC_UNDER_OR_OVERFLOW);
      });
      ...
      it("succeeds and saves updated values in storage on success", async () => {
        ...
        const expectedAccrualBlockNumberOrTimestamp = getBlockNumberOrTimestamp(isTimeBased) + 1;
        ...
        expect(await vToken.accrualBlockNumber()).to.equal(expectedAccrualBlockNumberOrTimestamp);
