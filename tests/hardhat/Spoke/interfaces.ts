 ...
 *
 * Every assertion here reads compiled ABIs and nothing else, so this file runs under the ordinary
 * `test` job. The assertions that call a live pool need a bscmainnet fork and stay in
 * `tests/hardhat/Fork/HubSpoke/interfaces.ts`, behind the fork gate. Keeping the two apart is what
 * makes the cross-repository guard fire on every pull request rather than only when someone runs
 * the fork suite by hand.
 */
...
describe("HubSpoke: the hub-side interfaces against the compiled contracts", () => {
