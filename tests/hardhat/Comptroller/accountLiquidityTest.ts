...
import {
  Comptroller, Comptroller__factory, PoolRegistry, AccessControlManager, CErc20Immutable, PriceOracle
} from "../../../typechain";
...
): Promise<FakeContract<CErc20Immutable>> {
  accessControl.isAllowedToCall.returns(true);
  const vToken = await smock.fake<CErc20Immutable>("CErc20Immutable");
  configureCToken({ vToken, comptroller, exchangeRate });
  if (supportMarket) {
...
function configureCToken({ vToken, comptroller, exchangeRate }: {
  vToken: FakeContract<CErc20Immutable>,
  comptroller: MockContract<Comptroller>,
  exchangeRate?: BigNumberish
}) {
  vToken.comptroller.returns(comptroller.address);
  vToken.isCToken.returns(true);
  if (exchangeRate) {
...
describe('Comptroller', () => {
  ...
  describe('liquidity', () => {
    it("fails if a price has not been set", async () => {
      const vToken = await makeCToken({accessControl, comptroller, oracle, supportMarket: true});
  ...
  describe("getHypotheticalAccountLiquidity", () => {
    it("returns 0 if not 'in' any markets", async () => {
      const vToken = await makeCToken({ accessControl, comptroller, oracle });
