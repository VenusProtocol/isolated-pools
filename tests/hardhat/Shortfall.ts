...
async function setup(isTimeBased: boolean) {
  ...
  await loadFixture(timeBasedhortfallFixture);
}

for (const isTimeBased of [false, true]) {
  const description: string = getDescription(isTimeBased);

  describe(`${description}Shortfall: Tests`, async function () {
      ...
      describe("waitForFirstBidder", async function () {
        ...
        it("emits WaitForFirstBidderUpdated event", async function () {
          ...
          if (!isTimeBased) {
            ...
            await expect(tx).to.emit(shortfall, "WaitForFirstBidderUpdated").withArgs(300, 200);
          }
        });
      });
      ...
      describe("updateNextBidderBlockLimit", async function () {
        ...
        it("emits NextBidderBlockLimitUpdated event", async function () {
          ...
          if (!isTimeBased) {
            ...
            await expect(tx).to.emit(shortfall, "NextBidderBlockLimitUpdated").withArgs(300, 110);
          }
        });
      });
    });
    ...
    describe("placeBid", async function () {
      ...
      async function startAuction() {
        ...
        auctionStartBlock = auction.startBlockOrTimestamp;
      }
      ...
      it("fails if auctionStartBlock does not match the auction startBlock", async () => {
        await startAuction();
        await mine(10);
        let latestBlockOrTimestamp = (await ethers.provider.getBlock("latest")).number;
        if (isTimeBased) {
          latestBlockOrTimestamp = (await ethers.provider.getBlock("latest")).timestamp;
        }
        await expect(shortfall.placeBid(poolAddress, "0", latestBlockOrTimestamp)).to.be.revertedWith(
    ...
    describe("LARGE_POOL_DEBT Scenario", async function () {
      ...
      it("Should not be able to placeBid when there is no active auction", async function () {
        ...
        await expect(shortfall.placeBid(poolAddress, "1800", auction.startBlockOrTimestamp)).to.be.revertedWith(
          "no on-going auction",
        );
      });
      ...
      it("Start auction", async function () {
        ...
        await shortfall.startAuction(poolAddress);
        const auction = await shortfall.auctions(poolAddress);
        startBlockNumber = auction.startBlockOrTimestamp;
      ...
      it("Should not be able to place bid lower max basis points", async function () {
        const auction = await shortfall.auctions(poolAddress);
        await expect(shortfall.placeBid(poolAddress, "10001", auction.startBlockOrTimestamp)).to.be.revertedWith(
      ...
      it("Place bid", async function () {
        ...
        await shortfall.placeBid(poolAddress, auction.startBidBps, auction.startBlockOrTimestamp);
      ...
      it("Should not be able to place bid lower than highest bid", async function () {
        const auction = await shortfall.auctions(poolAddress);
        await expect(shortfall.placeBid(poolAddress, "1200", auction.startBlockOrTimestamp)).to.be.revertedWith(
      ...
      it("Transfer back previous balance after second bid", async function () {
        ...
        await shortfall.connect(bidder2).placeBid(poolAddress, "1800", auction.startBlockOrTimestamp);
      ...
      it("Close Auction", async function () {
        const originalBalance = await mockDAI.balanceOf(bidder2.address);
        await mine((await shortfall.nextBidderBlockLimit()).toNumber() + 2);

        // simulate transferReserveForAuction
        await mockDAI.transfer(shortfall.address, parseUnits(riskFundBalance, 18));
        ...
        expect(await mockDAI.balanceOf(bidder2.address)).to.be.equal(originalBalance.add(auction.seizedRiskFund));
      });
    });
    ...
    describe("LARGE_RISK_FUND Scenario", async function () {
      ...
      it("Start auction", async function () {
        ...
        const newRiskFundBalance = "50000";
        fakeRiskFund.getPoolsBaseAssetReserves.returns(parseUnits(newRiskFundBalance, 18));

        await shortfall.startAuction(poolAddress);
        startBlockNumber = (await shortfall.auctions(poolAddress)).startBlockOrTimestamp;
        ...
        expect(new BigNumber(startBidBps).times(newRiskFundBalance).dividedBy(100).toString()).equal(
      ...
      it("Close Auction", async function () {
        ...
        // simulate transferReserveForAuction
        await mockDAI.transfer(shortfall.address, auction.seizedRiskFund);
        ...
        expect(await mockDAI.balanceOf(bidder2.address)).to.be.equal(originalBalance.add(riskFundBidAmount));
      });
    });
    ...
    describe("Restart Auction", async function () {
      ...
      it("Can restart auction", async function () {
        ...
        await shortfall.startAuction(poolAddress);
        const startBlockNumberOrTimestamp = (await shortfall.auctions(poolAddress)).startBlockOrTimestamp;
        ...
        await expect(shortfall.restartAuction(poolAddress))
          ...
          .withArgs(poolAddress, startBlockNumberOrTimestamp);
      });
  ...
  describe(`${description}Shortfall: Deflationary token Scenario`, async function () {
    let startBlockNumberOrTimestamp;
    ...
    it("Start auction", async function () {
      ...
      const newRiskFundBalance = "500";
      fakeRiskFund.getPoolsBaseAssetReserves.returns(parseUnits(newRiskFundBalance, 18));

      await shortfall.connect(owner).updateMinimumPoolBadDebt(convertToUnit(10, 18));

      await shortfall.startAuction(poolAddress);
      startBlockNumberOrTimestamp = (await shortfall.auctions(poolAddress)).startBlockOrTimestamp;
    ...
    it("Place bid", async function () {
      ...
      const tx = await shortfall
        .connect(bidder1)
        .placeBid(poolAddress, auction.startBidBps, auction.startBlockOrTimestamp);
    ...
    it("Transfer back previous balance after second bid", async function () {
      const auction = await shortfall.auctions(poolAddress);
      const tx = await shortfall.connect(bidder2).placeBid(poolAddress, "120", auction.startBlockOrTimestamp);
