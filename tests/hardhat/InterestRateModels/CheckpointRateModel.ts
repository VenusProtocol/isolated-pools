...
import { CheckpointRateModel, InterestRateModel } from "../../../typechain";
...
describe("CheckpointRateModel tests", () => {
  ...
  let oldRateModel: FakeContract<InterestRateModel>;
  let newRateModel: FakeContract<InterestRateModel>;
  let checkpointTimestamp: number;
  const cash = parseUnits("10", 18);
  const borrows = parseUnits("4", 18);
  const reserves = parseUnits("2", 18);
  const badDebt = parseUnits("1", 18);
  ...
  const fixture = async () => {
    ...
    oldRateModel = await smock.fake<InterestRateModel>(interestRateModelFactory.interface);
    newRateModel = await smock.fake<InterestRateModel>(interestRateModelFactory.interface);
  ...
  it("should use old rate model before checkpoint", async () => {
    ...
    expect(await checkpointRateModel.getBorrowRate(cash, borrows, reserves, badDebt)).to.equal(100);
    expect(await checkpointRateModel.getSupplyRate(cash, borrows, reserves, reserveFactorMantissa, badDebt)).to.equal(
      50,
    );
  });
  ...
  it("should use new rate model after checkpoint", async () => {
    ...
    expect(await checkpointRateModel.getBorrowRate(cash, borrows, reserves, badDebt)).to.equal(200);
    expect(await checkpointRateModel.getSupplyRate(cash, borrows, reserves, reserveFactorMantissa, badDebt)).to.equal(
      100,
    );
  });
