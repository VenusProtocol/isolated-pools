...
describe("CheckpointRateModel tests", () => {
  ...
  it("should use old rate model before checkpoint", async () => {
    ...
    expect(await checkpointRateModel.utilizationRate(cash, borrows, reserves, badDebt)).to.equal(1234);
  });
  ...
  it("should use new rate model after checkpoint", async () => {
    ...
    expect(await checkpointRateModel.utilizationRate(cash, borrows, reserves, badDebt)).to.equal(4321);
  });
