...
for (const isTimeBased of [false, true]) {
  ...
  describe(`${description}PoolLens`, async () => {
    ...
    describe(`${description}PoolLens - VTokens Query Tests`, async function () {
      ...
      it("is correct for WBTC as underlyingAsset", async () => {
        ...
        expect(vTokenMetadataActualParsed["supplyRatePerBlockOrTimestamp"]).equal("0");
        expect(vTokenMetadataActualParsed["borrowRatePerBlockOrTimestamp"]).equal("0");
    ...
    describe("Shortfall view functions", () => {
      ...
      it("getPoolBadDebt - all pools have debt", async () => {
        ...
        expect(resp.comptroller).to.be.equal(comptroller1Proxy.address);
        expect(resp.totalBadDebtUsd).to.be.equal("192821303636611822200000005");

        expect(resp.badDebts[1][0]).to.be.equal(vDAI.address);
        expect(resp.badDebts[1][1].toString()).to.be.equal("5");

        expect(resp.badDebts[0][0]).to.be.equal(vWBTC.address);
        expect(resp.badDebts[0][1].toString()).to.be.equal("192821303636611822200000000");
