...
describe("PoolLens", async function () {
  ...
  describe("Shortfall view functions", () => {
    ...
    it("getPoolBadDebt - all pools have debt", async () => {
      ...
      expect(resp.comptroller).to.be.equal(comptroller1Proxy.address);
      expect(resp.totalBadDebtUsd).to.be.equal("210003400000000000000000005000000000004556500");

      expect(resp.badDebts[1].vTokenAddress).to.be.equal(vDAI.address);
      expect(resp.badDebts[1].badDebtUsd.toString()).to.be.equal("5000000000004556500");

      expect(resp.badDebts[0].vTokenAddress).to.be.equal(vWBTC.address);
      expect(resp.badDebts[0].badDebtUsd.toString()).to.be.equal("210003400000000000000000000000000000000000000");
