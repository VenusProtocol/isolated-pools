...
contract LiquidationManager is ExponentialNoError {
    function calculateCloseFactor(
        ...
        if (healthFactor >= healthFactorThreshold) {
            // Prevent underflow
            require(
                wtAvg * totalCollateral <= borrowBalance * MANTISSA_ONE,
                "LiquidationManager: Collateral exceeds borrow capacity"
            );

            uint256 numerator = borrowBalance * MANTISSA_ONE - wtAvg * totalCollateral;
            uint256 denominator = borrowBalance *
                (MANTISSA_ONE - ((wtAvg * (MANTISSA_ONE + maxLiquidationIncentive)) / MANTISSA_ONE));

            closeFactor = (numerator * MANTISSA_ONE) / denominator;
            closeFactor = closeFactor > MANTISSA_ONE ? MANTISSA_ONE : closeFactor;
        } else {
            closeFactor = MANTISSA_ONE; // Liquidate 100% if unhealthy
        }
    }
